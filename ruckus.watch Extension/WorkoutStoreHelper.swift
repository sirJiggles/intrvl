...
protocol WorkoutManagerProtocol: class {
    func didStartWorkout(withConfiguration: HKWorkoutConfiguration)
}
...
class WorkoutStoreHelper: NSObject {
    ...
    var config: HKWorkoutConfiguration?
    ...
    public func startWorkout() {
        ...
        // get activity data and image etc to start the workout
        guard let activityData = activityHelper.activityData(), let activity = activityData["type"] as? HKWorkoutActivityType, let activityLocation = activityData["location"] as? String else {
            fatalError("Could not determine the type of workout")
        }
        
        workoutConfig.activityType = activity
        
        if activityLocation == "Indoor" {
