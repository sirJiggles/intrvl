...
class SettingsInterfaceController: WKInterfaceController {
    ...
    let sportPickerManager = PickerController()
    let locationPickerManager = PickerController()
    ...
    override func didAppear() {
        ...
        sportPickerManager.delegate = sportPicker
        sportPickerManager.setUpWithData(forKey: PossibleSetting.sport.rawValue)
        
        locationPickerManager.delegate = locationPicker
        locationPickerManager.setUpWithData(forKey: PossibleSetting.location.rawValue)
    ...
    // reset the sync button text
    func resetSyncButton() {
        // can come from notification cb
        DispatchQueue.main.async {
            self.syncButton.setEnabled(true)
            self.syncButton.setTitle("Sync with phone")
        }
    }
    ...
    @IBAction func didPickSport(_ value: Int) {
        sportPickerManager.didPick(index: value)
    }
    @IBAction func didPickLocation(_ value: Int) {
        locationPickerManager.didPick(index: value)
    }
    
    // for syncing workouts
    @IBAction func didTapSyncWorkout() {
        // sync the settings with the phone
        let defaults = UserDefaults.standard.dictionaryRepresentation()
        
        syncButton.setTitle("Syncing ...")
        syncButton.setEnabled(false)
        
        appNotificationBridge?.sendMessageWithPayload(.UserDefaultsPayloadFromWatch, payload: defaults, callback: resetSyncButton)
    }
